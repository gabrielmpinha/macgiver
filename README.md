...
MacGiver is a native macOS app for keeping your Mac awake, locking keyboard input while cleaning, controlling the built-in keyboard backlight, adjusting output volume, and checking battery and storage information. It lives in the menu bar, with compact summaries that expand into live readings, charts, and device controls.
...
| **Volume mixer** | Adjusts volume and mute state for output devices exposed by macOS. |
...
4. Use the **Volume** card to adjust the default output directly, or expand it to control each available output device.
...
Tests cover backlight state transitions, battery, storage, and volume decoding/control behavior, compiled translations, language selection, plurals, regional formatting, and rendered localization fixtures. System integrations use simulated hardware in unit tests; real output changes and menu bar interaction require the [manual verification steps](docs/development.md#hardware-and-ui-verification).
...
| CoreAudio | Output-device discovery, volume, and mute control |
